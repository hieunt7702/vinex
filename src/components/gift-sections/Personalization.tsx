    ...
    <section ref={container} className="py-16 lg:py-20 px-6 md:px-12 bg-vinex-black text-white overflow-hidden">
      <div className="max-w-[1536px] mx-auto">
        <div className="text-center mb-20">
          <h2 className="text-3xl md:text-5xl text-vinex-yellow mb-4 font-semibold">Dấu ấn độc bản</h2>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-8">
            {steps.map((step, idx) => (
              <div key={idx} className="pers-step relative flex flex-col items-center text-center group">
                ...
                <h3 className="text-sm md:text-base text-white group-hover:text-vinex-yellow transition-colors font-semibold">{step.title}</h3>
              </div>
            ))}
          </div>
        </div>
